...
export class PlayerVisuals {
  ...
  constructor(scene: THREE.Scene) {
    ...
    void document.fonts.load('19px Riffic').then(fonts => {
      this.nameFontReady = fonts.length > 0;
    }).catch(() => { /* Do not bake a fallback font into a persistent name texture. */ });
  }
  ...
  update(players: PlayerState[], localId: number, time: number, camera: THREE.Camera): void {
    ...
    visible.forEach((player, index) => {
      ...
      if (!name) {
        if (!this.nameFontReady) return;
        const canvas = document.createElement('canvas');
        canvas.width = 256; canvas.height = 40;
        const ctx = canvas.getContext('2d')!;
        ctx.font = '19px Riffic'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
