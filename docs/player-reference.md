...
Publié le 21 septembre 2026 avec le protocole 4 : [le bazooka RPG](bazooka.md) utilise un modèle polygonal facetté original créé dans le style de l'AK-47 et de l'AWP ; voir [la preuve de publication](deployment.md#publication-du-bazooka-assaut--21-septembre-2026). Sa silhouette et sa palette approuvées sont préservées. Le même OBJ/MTL sert en FPS et sur les personnages distants : 1 176 triangles, 656 sommets OBJ indexés, neuf matériaux, dont une lentille à 12 % d'opacité. Sa longueur reste 2,375 blocs. L'ogive olive séparée, `RPG_rocket`, utilise ses 280 triangles d'origine sur le lanceur et en vol. Elle disparaît du lanceur au tir, puis revient au terme de la cadence de 62 ticks.
...
Les formules de `HeadBone`, `*ArmBone`, `*ForeArmBone`, `*UpLegBone` et `*LegBone` sont reprises : tête en `sin(pitch) × 90°`, oscillations des bras, genoux articulés, cadence lente/rapide, poses fusil/outils et visée. La visée distante est alimentée par `PlayerState.aiming`, validé par le serveur pour AK/AWP/RPG. Le Java n'a pas d'animation séparée de saut, de mort, de tir ou de rechargement du squelette distant : le saut déplace ce même corps, la mort le masque. Les armes historiques suivent les mains calculées dans la même image.

Le RPG distant possède une pose adaptée à sa forme : tube abaissé à droite sur les avant-bras au repos, puis porté au-dessus de l'épaule droite en visée. Les deux bras restent articulés en segments de 0,5 bloc et leurs mains rejoignent les poignées du modèle ; la tête et le torse gardent leurs dimensions et animations historiques. Le tube pivote autour de l'appui d'épaule lorsqu'on vise, au lieu d'être attaché aux yeux. Cette pose est distincte du cadrage FPS à travers la lentille. `PlayerVisuals.shot` masque uniquement l'ogive pendant `62/60` seconde et ignore les confirmations répétées. `getRpgMuzzle` expose une copie de la pointe affichée : la roquette distante part visuellement de là puis rejoint la trajectoire serveur en 100 ms, sans modifier les collisions ni les dégâts. Un écart supérieur à 3 blocs n'est pas appliqué.
...
La révision RPG ajoute des tests sur les mains aux deux poignées, les coudes attachés, l'absence de sommets du modèle à l'intérieur du torse ou de la tête sur les poses repos/visée testées, le point de départ affiché et le retrait/retour de l'ogive. Le banc navigateur ajoute les vues repos, visée sur l'épaule et juste après le tir, avec comparaison des pixels pour l'ogive. Les résultats de la révision et ses captures sont consignés dans [la validation actuelle](validation.md). Les relevés ci-dessous concernent la reprise initiale des personnages Java.
...
Ajout du 14 septembre 2026, publié le 15 septembre : une mort confirmée crée un cadavre composé des dix volumes existants, simulés par cannon-es dans `src/client/ragdolls.ts`. Les neuf articulations conservent leurs pivots dans la pose affichée. La vitesse de déplacement est reprise, puis une impulsion est appliquée sur le volume orienté le plus proche du point touché. Le décalage entre ce contact et son centre produit un couple ; le mouvement ne se limite donc pas à une translation du torse.

Le serveur joint à l'événement JSON `death` une copie indépendante de l'état fatal, le point de collision et l'impulsion en coordonnées monde. Les tirs AK/AWP utilisent désormais la direction du rayon autoritaire, jamais une direction reconstruite depuis la position ultérieure du tireur. Les amplitudes cosmétiques sont 48 pour l'AK47, 80 pour l'AWP et 8 pour la pelle, pour une masse totale de 17,8. Marc a choisi le facteur ×4 lors de la revue vidéo du 15 septembre 2026, par rapport aux valeurs initiales 12/20. Les grenades appliquent au torse une impulsion radiale décroissante, plafonnée à 20 ; le RPG applique le même mécanisme à 80 %, plafonné à 16. Les morts environnementales n'ajoutent aucune impulsion. Ces réglages visuels ne représentent pas des masses balistiques réelles.
...
Les tirs AK/AWP suivants font aussi réagir les cadavres présents, avec une impulsion ×4 (48/80) et huit particules de sang identiques aux impacts sur un joueur vivant. Marc a choisi ×4 après le comparatif ×1/×2/×3/×4 du 20 septembre ; la première version utilisait 12/20. Le segment confirmé par le serveur est intersecté avec les volumes orientés de la pose affichée ; seul le premier membre rencontré reçoit l'impulsion au point touché, dans le sens du tir. Tous les membres reliés sont réveillés. La détection s'arrête à l'extrémité autoritaire du tir, donc avant un cadavre masqué par un bloc ou un joueur vivant. Un tir confirmé répété ne rejoue pas l'effet. Les cadavres restent traversables par les tirs de gameplay : aucune nouvelle blessure, élimination ni prolongation de leur durée de vie n'est produite. Cet ajout est publié le 20 septembre, sans modification du contrat réseau ni redémarrage serveur ; voir les preuves de publication dans deployment.md.

Limites retenues : les hitboxes de combat restent les AABB historiques ; le membre physique le plus proche du point confirmé est donc une approximation du membre visuel touché. Les articulations ont des limites coniques, sans butées anatomiques strictes de genou. Les membres et les cadavres ne se percutent pas entre eux et ne bloquent ni les joueurs ni les tirs. Chaque client simule sa propre pose finale ; aucun état persistant de cadavre n'est répliqué. L'ajout seul du champ optionnel `death` n'exigeait pas de changement de version ; le passage AK/AWP en hitscan porte ensuite le protocole à 3, sans changer le format binaire des snapshots.
