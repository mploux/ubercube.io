...
Si cette session CLI a expiré et possède encore son renouvellement privé, `bunx vercel@59.17.0 whoami --global-config .runtime/vercel-cli` permet au client officiel de la renouveler sans publier. Relancer ensuite `release:vercel status`. Cette récupération a été vérifiée le 15 septembre 2026 ; ne pas confondre une expiration avec l'absence de droits sur le projet et ne pas afficher le fichier d'authentification.
