...
À la mort, le client suit son ragdoll pendant 3 s, avec caméra décalée et rapprochée devant les voxels qui l'obstruent. L'ajustement local du 22 septembre (non publié) porte ensuite la kill cam de 3 à 5 s depuis les yeux du tueur : 2,7 s avant la mort, 2 s de continuation à vitesse réelle, puis 0,3 s de maintien final. Positions, visées, projectiles, tirs et impacts sont reconstruits depuis les messages reçus. Le serveur ajoute seulement une copie de la pose du tueur à l'événement de mort existant ; aucun enregistrement vidéo ni historique serveur. Les tirs rejoués déclenchent uniquement rendu et audio, sans envoyer de commandes ni appliquer de dégâts.
...
L'ajustement local du 22 septembre conserve la capture souris entre jeu, death cam et kill cam ; retour au lobby, Échap ou perte de focus la libèrent. Le clic droit et la molette ne déclenchent pas d'action navigateur pendant les caméras. Sur ordinateur, choisir le kit ou reprendre demande la capture clavier et le plein écran : Ctrl reste la course et Tab affiche les scores, y compris ensemble. Échap libère les captures et ouvre le menu en jeu. Le lobby et les champs de réglage conservent leurs entrées normales. La capture clavier est libérée à la pause, à la perte de focus/pointeur, à la sortie du plein écran et au retour au lobby. Mobile inchangé.

Le navigateur peut refuser ces demandes ou ne pas proposer Keyboard Lock ; un message indique alors que certains raccourcis restent actifs. La capture des combinaisons réservées demande le plein écran via l'API, et les raccourcis système restent soumis aux limites du navigateur/OS : [documentation Chrome](https://developer.chrome.com/docs/capabilities/web-apis/keyboard-lock). La capture du pointeur doit être demandée avant le plein écran, qui consomme l'activation du clic : [documentation Pointer Lock](https://developer.mozilla.org/en-US/docs/Web/API/Element/requestPointerLock#security). Ces corrections restent côté client, sans changement du protocole 6.
