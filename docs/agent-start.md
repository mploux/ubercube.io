...
Les identifiants ne sont jamais fournis par le dépôt : GitHub utilise l'accès Git configuré, Vercel un token privé, SSH une clé autorisée/agent ou une saisie interactive. Une nouvelle machine sans ces accès peut tout développer et tester ; l'accès au déploiement doit être provisionné séparément.
