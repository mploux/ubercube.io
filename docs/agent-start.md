...
Le produit validé : TDM ou FFA, jusqu'à 100 joueurs par défaut, pseudo sans compte, équipement puis apparition, AK-47/AWP en hitscan et grenades physiques, outils de construction/destruction, réinitialisation par manche. Le hitscan et les ragdolls ont été publiés le 15 septembre 2026 avec le protocole 3 côté client et serveur ; voir les preuves dans docs/deployment.md. L'UI, la police, le HUD, la minicarte, les personnages et armes reprennent le Java. Conserver l'accueil avec panorama, le lobby historique, les contrôles à deux pouces et le gain audio global `/50`. Les sensations humaines, la compensation historique des impacts, les grandes cartes, les effondrements et le scaling distribué restent des sujets ouverts ; aucune DB n'est nécessaire au périmètre actuel.

Le chantier local à 100 joueurs utilise désormais **protocole 7 / format binaire 5** : registre fiable des identités conservé au reset, snapshots différentiels à 20 Hz avec mouvements sélectionnés par destinataire à 5/10/20 Hz, dates de capture individuelles et priorité sniper à toute distance dans la zone avant. L'état privé et les corrections de vitesse des grenades restent réservés au propriétaire ; `roundEndTick` donne l'échéance et le payload de mort est limité au visuel. Voir [le contrat et les budgets](stable-100.md). Les mesures de charge des protocoles précédents restent historiques et ne qualifient pas cette évolution. Consulter [les résultats datés](validation.md) avant de conclure sur la capacité validée.
...
- **Qualification locale** : `bun run qualify:local --players=100 --seconds=300 --output=.runtime/qualification-100.json` crée et ferme son propre serveur local dans un processus séparé, compare aussi le terrain autoritaire et configure l'admission nécessaire aux bots d'une seule IP. Pour une instance manuelle, ajouter `--max-connections-per-ip=116` au lancement serveur. `--memory-profile` produit un diagnostic mémoire distinct de l'endurance ; créer `.runtime/qualification-100.json.stop` arrête proprement cet essai et conserve ses rapports. Voir [budgets et critères](stable-100.md).
