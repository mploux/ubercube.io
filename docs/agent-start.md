...
Le chantier local à 100 joueurs utilise désormais **protocole 6 / format binaire 4** : registre fiable des identités conservé au reset, snapshots publics différentiels à 20 Hz, état privé et corrections de vitesse des grenades réservés au propriétaire, échéance `roundEndTick` et payload de mort limité au visuel. Voir [le contrat et les budgets](stable-100.md). Les mesures de charge en protocole 5 restent historiques : l'essai interrompu volontairement après environ 43,4 minutes, avec nettoyage confirmé, ne constitue pas une qualification de huit heures ni du nouveau protocole. Consulter [les résultats datés](validation.md) avant de conclure sur la capacité validée.
