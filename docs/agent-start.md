...
- **Qualification locale** : `bun run qualify:local --players=100 --seconds=300 --output=.runtime/qualification-100.json` crée et ferme son propre serveur local dans un processus séparé, compare aussi le terrain autoritaire et configure l'admission nécessaire aux bots d'une seule IP. Pour une instance manuelle, ajouter `--max-connections-per-ip=116` au lancement serveur. Voir [budgets et critères](stable-100.md).
