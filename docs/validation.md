...
Préparation Vercel + serveur Bun séparé : `bun run check` passe avec 176 tests et 8804 assertions. Les nouveaux tests couvrent les adresses client HTTP/WSS, les origines autorisées et refusées, les requêtes HTTP réelles et les upgrades WebSocket. Les builds Vercel refusent une adresse serveur absente ou HTTP ; un build HTTPS incorpore correctement l'adresse configurée. Un navigateur réel a chargé le client statique sur le port 3013, obtenu l'état serveur puis rejoint le lobby du serveur Bun sur le port 3012 sans erreur CORS/JavaScript. Le build local a ensuite été restauré ; les 385 fichiers Java restent intacts. L'authentification Vercel est maintenant validée et le projet `ubercube-io` est créé ; ses paramètres de compilation ont été relus via l'API. Aucun déploiement n'est publié : l'hébergement du serveur permanent reste à fournir. Le Dockerfile est fourni, mais l'image n'a pas été construite faute de Docker local.
