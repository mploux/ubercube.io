...
La simulation avance à 60 Hz ; les états sont diffusés à 20 Hz dans un format binaire. Le candidat local encode aussi les événements fréquents et les mutations de terrain en binaire ; les intentions, identités, messages de cycle de vie et morts utilisent JSON. L'identité, le temps de simulation, les impacts, la vie et les ressources sont décidés par le serveur. Les tailles, fréquences, files d'entrée et buffers réseau sont bornés.

Les intentions sont envoyées dès l'image qui les produit, sans attendre trois commandes. Après une pointe réseau, le serveur regroupe les intentions identiques déjà maintenues pour résorber le retard ; chaque appui, relâchement et changement d'arme conserve son ordre et son identifiant, avec une seule simulation par tick. Les personnages distants utilisent une interpolation individuelle sur leur date de capture serveur, avec un tampon de 50, 100 ou 200 ms selon leur cadence, augmenté selon la gigue observée. Une interruption réseau conserve la dernière position confirmée.
...
Le candidat local à 100 joueurs utilise le protocole 7 / binaire 5 : snapshots différentiels à 20 Hz, identités conservées par connexion et corrections privées envoyées au propriétaire. Les mouvements distants sont actualisés par destinataire à 5, 10 ou 20 Hz selon le regard et la distance ; l'AWP garde la zone avant à 20 Hz quelle que soit la distance. Santé, munitions, classe, stock exact de grenades et acquittements ne figurent plus dans les états publics. Le chronomètre découle du tick de fin de manche. Voir [le contrat réseau et ses limites](docs/stable-100.md), puis [les résultats effectivement mesurés](docs/validation.md). Cette évolution nécessite une publication coordonnée du client et du serveur ; la production enregistrée reste en protocole 3.

L'essai local de dix minutes du 16 septembre réduit le trafic des snapshots de 30,2 % et la sortie totale de 12,3 %, sans erreur ni tick abandonné à 100 bots. Le maximum des p99 serveur échantillonnés atteint toutefois 8,417 ms : le critère de 8 ms échoue et la qualification de stabilité reste ouverte, ainsi que l'endurance mémoire.
